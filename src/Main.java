...
public class Main {
	public static void main(String[] args) {
		...
		Event event1 = new Event("Robotics Workshop", "Technology",
				"Learn the basics of building and programming robots.", "Thessaloniki International Fair", 500, 8, 3,
				2025, 8, 30, 160, org1, "Pending");

		Event event2 = new Event("Wine Tasting - Local Vineyards", "Culinary",
				"Discover the finest wines from local vineyards.", " Thessaloniki Wine Cellar", 200, 15, 5, 2025, 10, 0,
				120, org1, "Approved");

		Event event3 = new Event("Fashion Show - Fall Collection", "Fashion",
				"A stunning presentation of the latest fall trends.", "Thessaloniki Art Center", 300, 10, 6, 2025, 19,
				30, 180, org2, "Pending");

		Event event4 = new Event("Yoga Retreat - Wellness and Relaxation", "Health",
				"A weekend of yoga, meditation, and relaxation.", "Thessaloniki Concert Hall", 150, 12, 6, 2025, 18, 0,
				120, org2, "Approved");

		Event event5 = new Event("Startup Expo - Innovation and Networking", "Business",
				"Meet the most innovative startups and network with entrepreneurs.", " Thessaloniki International Fair",
				100, 5, 7, 2025, 9, 0, 180, org3, "Pending");

		Event event6 = new Event("Classical Music Concert - Beethoven's Symphony", "Music",
				"An orchestra performing Beethoven's greatest symphonies.", "Thessaloniki Concert Hall", 50, 8, 7, 2025,
				17, 0, 120, org3, "Approved");

		Event event7 = new Event("Marathon - Run for a Cause", "Sports",
				"Participate in a marathon to support a charity.", "Thessaloniki Waterfront", 400, 20, 8, 2025, 10, 0,
				240, org4, "Pending");

		Event event8 = new Event("Science Fair - Discover the Future", "Art",
				"Explore interactive exhibits and talks from leading scientists.", "Thessaloniki Science Center", 500,
				22, 8, 2025, 18, 0, 300, org4, "Approved");

		// Δημιουργία event με μικρή χωρητικότητα για δοκιμή
		Event limitedEvent = new Event("Exclusive Art Tour", "Art", "VIP tour of the art gallery.",
				"Thessaloniki Art Center", 2, 1, 1, 2025, 10, 0, 120, org1, "Approved");
		...
		// Δημιουργία κράτησης
		Reservation.createReservation(visitor1, limitedEvent); // Επιτυχής κράτηση
		Reservation.createReservation(visitor2, limitedEvent); // Επιτυχής κράτηση
		Reservation.createReservation(visitor3, limitedEvent); // Αποτυχία: Δεν υπάρχουν διαθέσιμες θέσεις

		// Ακύρωση κράτησης
		Reservation.cancelReservation(visitor1, limitedEvent);
		...
		// Για να διαγράψω μια εκδήλωση απο τους άλλους δύο διοργανωτές πρέπει πρώτα να
		// προσθέσω σε αυτούς κάποια εκδήλωση
		...
		org4.addEvent(new Event("Book Fair", "Literature", "Explore new and classic books", "Thessaloniki Concert Hall",
				400, 20, 8, 2025, 10, 0, 240, org4, "Pending"));
		...
		if (!org3.getEvents().isEmpty()) {
			...
			System.out.println(
					"Η εκδήλωση " + eventToDelete1.getTitle() + " διαγράφηκε από τον οργανωτή " + org3.getName() + ".");
		} else {
		...
		if (!org4.getEvents().isEmpty()) {
			...
			System.out.println(
					"Η εκδήλωση " + eventToDelete2.getTitle() + " διαγράφηκε από τον οργανωτή " + org4.getName() + ".");
		} else {
			System.out.println("Η λίστα εκδηλώσεων για τον οργανωτή " + org4.getName() + " είναι κενή.");
		}
		...
		if (searchResults.isEmpty()) {
			...
			for (Event event : searchResults) {
				...
				System.out.println("Time: " + event.getHour() + ":" + event.getMinutes()); // Formatting the minutes
		...
		if (chosenEvent != null) {
			boolean success = Reservation.createReservation(visitor, chosenEvent); // Χρήση της Reservation
			if (!success) {
		...
		if (cancelChoice.equalsIgnoreCase("yes")) {
			...
			// Εύρεση της εκδήλωσης που θέλει να ακυρώσει
			for (Reservation reservation : Reservation.getAllReservations()) { // Παίρνει όλες τις κρατήσεις
				if (reservation.getVisitor().equals(visitor)
						&& reservation.getEvent().getTitle().equalsIgnoreCase(cancelEventTitle)) {
			...
			if (cancelEvent != null) {
				boolean success = Reservation.cancelReservation(visitor, cancelEvent); // Χρήση της Reservation
				if (success) {
					System.out.println("Η κράτηση ακυρώθηκε επιτυχώς.");
				} else {
					System.out.println("Η ακύρωση απέτυχε.");
				}
			} else {
