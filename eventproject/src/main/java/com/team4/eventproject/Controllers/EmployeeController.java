...
public class EmployeeController {
	...
	@PostMapping("/process")
	public ResponseEntity<String> handleApprovalRequest(@RequestParam Long approvalRequestId,
			@RequestParam String status, @RequestParam String comments, @RequestParam Long employeeId) {
		try {
	...
	public ResponseEntity<String> deleteEventDirectly(@RequestBody Event event, // Η εκδήλωση που θέλουμε να
																				// διαγράψουμε.
			@RequestParam Long organizerId, @RequestBody Employee employee)// Ο υπάλληλος που εκτελεί τη διαγραφή.
	{
		...
		if (result) {
			...
			return ResponseEntity.badRequest()
					.body("Η εκδήλωση '" + event.getTitle() + "' δεν βρέθηκε στη λίστα του διοργανωτή.");
		}
	}
